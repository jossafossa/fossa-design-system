...
export const Clickable = ({ className, ...props }: PropsWithChildren<ClickableProps>) => {
  const classes = classNames(styles.clickable, className);

  function isLinkProps(props: PropsWithChildren<ClickableProps>): props is LinkProps {
    return ['to', 'href'].some((prop) => prop in props);
  }

  function isButtonProps(props: PropsWithChildren<ClickableProps>): props is ButtonProps {
    return 'onClick' in props;
  }

  if (isLinkProps(props)) return <Link className={classNames(classes, styles.anchor)} {...props} />;
